...
export function Hero() {
  return (
    <header className="relative flex min-h-screen items-center overflow-hidden">
      <HeroBackground />
      <div className="relative z-[2] mx-auto grid w-full max-w-6xl grid-cols-1 items-center gap-14 px-7 py-28 lg:grid-cols-[1.05fr_0.95fr]">
        <div>
          <Reveal>
          ...
          <Reveal delay={0.05}>
            <h1 className="font-display text-[clamp(44px,7.5vw,92px)] font-bold leading-[0.98] tracking-tight">
              {profile.name}
            </h1>
          </Reveal>
          <Reveal delay={0.1}>
            <p className="mt-3 font-display text-[clamp(20px,3vw,32px)] font-semibold">
